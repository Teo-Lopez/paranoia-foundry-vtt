...
export class ParanoiaActor extends Actor {
  ...
  _calcSkillsValues(actorData) {
    ...
    for (let [_groupName, group] of Object.entries(data.skills)) {
      for (let key in group.commonSkills) {
        const skill = group.commonSkills[key];

        skill.value =
          group.value +
          Object.values(skill.isSpecialty).filter((e) => e === true).length * 4;

        if (skill.isVital) {
          skill.value += 4;
        }

        group.commonSkills[key] = skill;
        console.log(skill.isWeakness);
        if (skill.isWeakness) {
          console.log(skill);
          skill.value = 1;
        }
  ...
  _calcTotalSkillPoints(actorData) {
    const data = actorData.data;
    for (let group in data.skills) {
      data.skillPoints.value -= data.skills[group].value;
    }
  }
